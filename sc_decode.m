...
N = 1024;% Block length 
K = 512; % Message bits + CRC padded bits
n = log2(N); % taking log of block length
EbNodB = 2;%range of values representing the signal strength in the channel
Rate = K/N; %code rate 
EbNo = 10^(EbNodB/10);
sigma = sqrt(1/(2*Rate*EbNo)); 

Q1 = Q(Q<=N); %reliability sequence for N
% in the decoder, we have to check if a particular bit is frozen
F = Q1(1:N-K); %Frozen positions: Q1(1:N-K)
% Thus the remaining would be,Message positions: Q1(N-K+1:end)
%disp(F);
...
for blk = 1:Nblocks
 ...
 u = zeros(1,N);% our input vector u 
    
 u(Q1(N-K+1:end)) = msg; %assigning the remaining bits to message after
                             % assigning the frozen bits to zero
 ...
 for i = 1:2*m:N
  ...
  u(i:i+2*m-1) = [mod(a+b,2) b]; %combining XOR function 
   end
   m = m * 2;
   end
   cword = u;
  ...
  r = s + sigma * randn(1,N); %AWGN channel I, r is the recieved value 
    
    %%
   % disp(msg);
    %SC decoder
    L = zeros(n+1,N); % storage for beliefs
    ucap = zeros(n+1,N); % decisions which will be sent back to parent node by children
    ns = zeros(1,2*N-1); 
    %node state vector : we have 4 node states 0,1,2,3
    % 0: yet to be activated and once activated do Step L  ; 1 : sent
    % belief to right child ; 2 : recieved decisions from both children 
    ...
    while (done == 0) %traverse till all bits are decoded
       ...
       if node == (N-1)
            ...
            temp = 2^(n-depth);
             ...
             L(depth+1,temp*node+1:temp*(node+1)) = f(a,b); %minsum calculation and storage
             ns(npos) = 1; % update the node state to 1 
            else
                if ns(npos) == 1 %step R and go to right child
                 ...
                 a = Ln(1:temp/2); b = Ln(temp/2+1:end); %split beliefs into 2
                  ...
                  temp = temp / 2; %incoming belief length for right child
                   ...
                   ns(npos) = 2; % update to node state 2 
    ...
    msg_cap = ucap(n+1,Q1(N-K+1:end));% msg incoming decisions from the children 
